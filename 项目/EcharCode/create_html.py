...
# 获取排名前十的影片
def movie_score_top_ten(html_name="Top10影片名称及评分.html"):

    render_path = "./templates/" + html_name
...
def movie_score_comment(html_name="影片评分及评论数.html"):
...
def movie_comment_num(html_name="各年代影片数量及评论数量.html"):
...
def movie_director_actors(html_name="导演,演员词云图.html"):
    render_path = "./templates/" + html_name
    db = MysqlDB(**DB_CONFIG)
    db.connection()
...
def movie_type(html_name="影片类型.html"):
    render_path = "./templates/" + html_name
...
def movie_time(html_name="电影时长.html"):
    render_path = "./templates/" + html_name
    x_data = [
        ("45-93分钟"),
        ("94-141分钟"),
